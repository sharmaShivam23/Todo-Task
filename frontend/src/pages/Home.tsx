...
const Home = () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (_newTodo, _variables, _context) => {
      queryClient.setQueryData(['todos'], (oldTodos: Todo[] = []) => {
        const filtered = oldTodos.filter(todo => !todo._id.startsWith('temp-'));
        return [_newTodo, ...filtered];
      });
      toast.success('Todo created successfully!');
      reset();
    },
    onError: (_err, _variables, _context) => {
      if (_context?.previousTodos) {
        queryClient.setQueryData(['todos'], _context.previousTodos);
      }
      toast.error('Failed to create todo');
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (_updatedTodo) => {
      queryClient.setQueryData(['todos'], (oldTodos: Todo[] = []) =>
        oldTodos.map((todo) => (todo._id === _updatedTodo._id ? _updatedTodo : todo))
      );
      toast.success('Todo updated successfully!');
      setEditingId(null);
    },
